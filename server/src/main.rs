...
async fn main() {
    ...
    let db_address =
        std::env::var("SURREALDB_ADDRESS").unwrap_or_else(|_| "localhost:8000".to_string());
    let namespace =
        std::env::var("SURREALDB_NAMESPACE").unwrap_or_else(|_| "escrow".to_string());
    let database =
        std::env::var("SURREALDB_DATABASE").unwrap_or_else(|_| "wallet".to_string());
...
/// `POST /api/transactions` — record a transaction for the wallet. JWT-gated:
/// the `Claims` extractor rejects the request unless a valid token is present.
...
/// `GET /api/transactions/:wallet_id` — the wallet's transaction history. JWT-gated.
